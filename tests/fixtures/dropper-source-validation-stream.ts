...
/** Drives the production agentLoop through exact dropper tool calls and terminal responses. */
export function scriptedDropperStream(steps: DropperStreamStep[]) {
	let callCount = 0;
	...
	const streamSimple = (model: any, context: any) => {
		...
		if (!step) throw new Error(`Unexpected dropper stream call ${callCount}`);
		const stream = createAssistantMessageEventStream();
		queueMicrotask(() => {
			const output = assistant(model);
			stream.push({ type: "start", partial: output } as any);
			if ("toolName" in step) {
				const toolCall = { type: "toolCall", id: `tool-${callCount}`, name: step.toolName, arguments: step.arguments };
				...
				const text = { type: "text", text: "Dropper pass complete." };
