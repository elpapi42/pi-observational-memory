...
export type ObserverStreamStep =
	| {
		toolName: "validate_source_entry_ids" | "record_observations";
		arguments: unknown;
...
/** Drives the production agentLoop through an exact sequence of tool calls and terminal responses. */
export function scriptedObserverStream(steps: ObserverStreamStep[]) {
	let callCount = 0;
	...
	const streamSimple = (model: any, context: any) => {
		...
		if (!step) throw new Error(`Unexpected observer stream call ${callCount}`);
