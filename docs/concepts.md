...
An **observation** is a single timestamped event from the conversation. One short sentence of plain prose, plus three pieces of metadata:
...
Observations are written **continuously** by a background process called the **observer** as you work. They accumulate in the session tree. Over a long session, you'll have hundreds of them.

### Reflections

A **reflection** is a stable, long-lived fact distilled from many observations. Just plain prose — no timestamp, no relevance tier, no prefix.

```
User works at Acme Corp building Acme Dashboard on Next.js 15.
```

A reflection names a **durable pattern**, not a specific event. It's the kind of fact that should still be true thirty turns from now: who the user is, what they're building, hard constraints, architectural decisions. Once a reflection is recorded, it persists across every future compaction.
...
When the conversation accumulates enough raw tokens (default: ~50k since the last compaction), the extension calls Pi's `compact()`. This replaces the older messages in context with a single **compaction summary**.

Crucially: the summary itself is **not** written by an LLM. It's a deterministic concatenation of all current reflections followed by all current observations. This is what eliminates the "summary of a summary of a summary" degradation — what survives one compaction survives all of them, byte-identical.
...
- The **reflector** reads the full pool and crystallizes new long-lived patterns into new reflection lines. It's append-only — it never rewrites or removes existing reflections.
- The **pruner** drops observations from the pool by id. It runs up to 5 passes, getting more aggressive each time, until the pool fits under ~80% of the budget. It cannot rewrite or merge observations — only drop them.
...
Below the gate, both are skipped. Compaction in that case is **zero LLM calls** — just bookkeeping.
...
| Term | Meaning |
|---|---|
| **Observation** | One timestamped, relevance-tagged event. Plain prose. Written by the observer. |
| **Reflection** | One durable pattern. Plain prose, no timestamp. Crystallized by the reflector. |
| **Observer** | Async LLM agent that runs every ~1k raw tokens in the background, writing observations. |
| **Reflector** | LLM agent that runs at compaction (above the gate) to crystallize new reflections from observations. Append-only. |
| **Pruner** | LLM agent that runs after the reflector to drop observations by id. Cannot rewrite, only drop. |
...
| **Last bound** | The more recent of the most recent observation entry and the most recent compaction. The cutoff the observer trigger walks back to. |
